...
def handle_http_get_hello(req, conn):
    ...
    if favcolor not in [ "coral", "crimson", "navy", "purple", "tomato", "teal", "wheat" ]:
        favcolor = None
    msg = '<html><head><title> Welcome, %s!</title></head>' % (name)
    msg += '<body>'
    #msg = f"Welcome, {name}!\n"
    msg += '<p>The current date is %s.</p>' % (time.strftime("%a, %d %b %Y %H:%M:%S %Z"))
    msg += '<p>You have visited this page an unknown number of times before.</p>'
    msg += '<p>Maybe your favorite color is... %s?!</p>' % (favcolor)
    msg += '<p>Hit page refresh (F5) to refresh this page and see the contents MOSTLY STAY THE SAME?? </p>'
    msg += '<p>You can also go to these exciting pages:</p>'
    msg += '    <pre><a href="http://localhost:8888/hello"> refresh this page</a>        - this page</pre>'
    msg += '    <pre><a href="http://localhost:8888/status"> status and statistics</a></pre>'
    msg += '    <pre><a href="http://localhost:8888/quote"> a random quote generator</a></pre>'
    msg += '    <pre><a href="http://localhost:8888/welcome.html"> a welcome page</a></pre>'
    msg += '    <pre><a href="http://localhost:8888/index.html"> some fancy web page</a></pre>'
    #msg += "\n"
    msg += "<p>Enter your name: ___________</p>"
    msg += "<p>Favorite color: ___________</p>"
    msg += "<p>Just kidding, this page isn't yet interactive. Sorry.</p>"
    msg += '</body></html>'
...
def handle_http_get_file(url_path):
    ...
    try:
        with open(file_path, "rb") as f: # "rb" mode means read "raw bytes"
            data = f.read()
            # reads end of url, selects correct mime type to acompany
            # .jpeg AND .jpg          
            if url_path.lower().endswith(".jpeg") or url_path.lower().endswith(".jpg"): # lower()  makes all varations become lowercase 
                mime_type = "image/jpeg"
            # .png
            elif url_path.lower().endswith(".png"):
                mime_type = "image/png"
            #.txt
            elif url_path.lower().endswith(".txt"):
                mime_type = "text/plain"
            #.css
            elif url_path.lower().endswith(".css"):
                mime_type = "text/css"
            #.js
            elif url_path.lower().endswith(".js"):
                mime_type = "text/javascript" # might need to edit this one
            # .html AND .htm
            elif url_path.lower().endswith(".html") or url_path.lower().endswith(".htm"):
                mime_type = "text/html" # now it should be right (i hope)
        return Response("200 OK", mime_type, data)
    except:
        log("Error encountered reading from file")
        return Response("403 FORBIDDEN", "text/plain", "Permission denied: " + url_path)

# handle_http_get_whoami() returns info about the client obtained from the client's own request. 
def handle_http_get_whoami(req, conn):
    ...
    accepted_language = get_header_value(req.headers, "Accept-Language")
    ...
    msg += f"<p><strong>Accept-Language:</strong> {accepted_language}</p>"
    msg += f"<p><strong>Cookie:</strong> {cookies}</p>"
    msg += f"<p><strong>DNT (Do-Not-Track):</strong> {dnt}</p>"
    msg += f"<p><strong>Sec-GPC (Global Privacy Control):</strong> {sec_gpc}</p>"
    msg += "</body>"
    msg += "</html>"
    return Response("200 OK", "text/html", msg)
